...
let player, bullets, enemies, particles, explosions, enemyBullets, pickups, obstacles, turrets, traps, camera, score, credits, wave, kills, running, last, spawnTimer, eliteRespawn, bossTriggered, bossSpawnTimer, bossSpawnPoint, bossPurgeTimer, bossFightActive, shake, flash, notice, noticeTime, visualTime;
...
function reset(){
  spaceDashQueued=false;turretQueued=false;healQueued=false;
  shopOpen=false;turretHpMultiplier=1;playerDamageMultiplier=1;healthRegen=false;shopScreen.classList.add('hidden');
  const maxHp=chosenRole==='tank'?125:chosenRole==='assassin'?75:100;
  player={x:WORLD_W/2,y:WORLD_H/2,r:17,hp:maxHp,maxHp,speed:chosenRole==='assassin'?390:260,angle:0,moveX:1,moveY:0,fire:0,dash:0,inv:0,heal:0,invisible:0,ghostCooldown:0,manipulateCooldown:10,manipulateCharges:3,manipulateReady:false,traps:10,trapRecharge:10,reflect:0,ninjaCooldown:0,madmanCooldown:0};
  bullets=[]; enemies=[]; particles=[];explosions=[]; enemyBullets=[]; pickups=[];turrets=[];traps=[];
...
function placeTurret(){
  ...
  if(turrets.filter(t=>t.hp>0).length>=11){notice=tr('LIMIT 11 WIEŻYCZEK OSIĄGNIĘTY','11 TURRET LIMIT REACHED');noticeTime=1.5;return}
  ...
  if(turrets.filter(t=>t.hp>0).length>10&&!bossTriggered)beginBossArrival();
}
...
function spawnEnemy(){
  ...
  enemies.push({x,y,r:tank?25:18,hp:tank?5:2,maxHp:tank?5:2,speed:tank?rand(55,75):rand(75,115)+wave*2,shoot:rand(.5,1.8),angle:0,tank,hit:0});
}
...
function update(dt){
  ...
  player.fire-=dt; player.dash-=dt; player.inv-=dt;player.heal-=dt;player.invisible-=dt;player.ghostCooldown-=dt;player.reflect-=dt;player.ninjaCooldown-=dt;player.madmanCooldown-=dt; flash-=dt;noticeTime-=dt; shake*=.88;
  ...
  if(mouse.down&&player.fire<=0){
    player.fire=.115; const a=player.angle+rand(-.025,.025);
    const special=chosenRole==='manipulator'&&player.manipulateReady;
    const shotAngles=chosenRole==='madman'?[a-.055,a+.055]:[a];
    shotAngles.forEach(shotAngle=>bullets.push({x:player.x+Math.cos(shotAngle)*25,y:player.y+Math.sin(shotAngle)*25,vx:Math.cos(shotAngle)*720,vy:Math.sin(shotAngle)*720,life:1.2,special}));
  ...
  if(!bossFightActive){
    spawnTimer-=dt;
    if(spawnTimer<=0){ spawnEnemy(); spawnTimer=Math.max(.28,1.18-wave*.07)*rand(.75,1.2); }
  ...
  turrets.forEach(t=>{
    ...
    if(target){
      ...
      if(t.fire<=0){const a=t.angle;t.fire=.38;bullets.push({x:t.x+Math.cos(a)*26,y:t.y+Math.sin(a)*26,vx:Math.cos(a)*610,vy:Math.sin(a)*610,life:1.2,turret:true});burst(t.x+Math.cos(a)*25,t.y+Math.sin(a)*25,'#58ffd1',2)}
    }
  });

  enemies.forEach(e=>{
    e.hit-=dt;if(e.elite)e.dodge-=dt;
    if(e.friendly){
    ...
    if(e.shoot<=0&&d<650){
      ...
      e.shoot=(e.elite?1.15:e.tank?1.5:1.9)*rand(.8,1.2)/Math.min(1.5,1+wave*.03);
    }
  });
  ...
  for(const b of bullets)for(const e of enemies)if(b.life>0&&e.hp>0&&!e.friendly&&dist(b,e)<e.r+4){
    ...
    e.hp-=(playerShot?(chosenRole==='assassin'?2:1)*playerDamageMultiplier:1);e.hit=.08;score+=20;burst(b.x,b.y,b.turret?'#58ffd1':'#eaffac',5);
  ...
  for(const b of enemyBullets){
    ...
    if(b.life>0){
      ...
      if(turret){b.life=0;turret.hp--;turret.hit=.18;shake=6;burst(turret.x,turret.y,'#ff9d3d',12);if(turret.hp<=0){burst(turret.x,turret.y,'#58ffd1',28);notice=tr('WIEŻYCZKA ZNISZCZONA','TURRET DESTROYED');noticeTime=1.4}}
    }
    if(b.life>0&&player.inv<=0&&!(chosenRole==='ghost'&&player.invisible>0)&&dist(b,player)<player.r+b.r){b.life=0;player.hp-=b.damage;player.inv=.45;player.speed=chosenRole==='assassin'?390:260;shake=b.damage===36?5:12;flash=b.damage===36?.06:.12;burst(player.x,player.y,'#ff405d',b.damage===36?10:18);if(player.hp<=0)endGame()}
  }
  ...
  enemies=enemies.filter(e=>e.hp>0);turrets=turrets.filter(t=>t.hp>0);traps=traps.filter(t=>!t.used);particles=particles.filter(p=>p.life>0);explosions=explosions.filter(e=>e.life>0);pickups=pickups.filter(p=>p.life>0);
}
...
function roleHud(){
  const names={medic:tr('TECHNIK','TECHNICIAN'),tank:'TANK',billionaire:tr('MILIARDER','BILLIONAIRE'),assassin:tr('ASSASYN','ASSASSIN'),ghost:tr('DUCH','GHOST'),manipulator:'MANIPULATOR',engineer:tr('INŻYNIER','ENGINEER'),ninja:'NINJA',madman:tr('SZALENIEC','MADMAN')};
...
function buyTurretHpUpgrade(){
  ...
  turrets.forEach(t=>{t.hp*=2;t.maxHp*=2});
  toggleShop(true);
}
...
function draw(){
  ...
  turrets.forEach(t=>{ctx.save();ctx.translate(t.x,t.y);ctx.rotate(t.angle);ctx.shadowBlur=12;ctx.shadowColor=t.hit>0?'#fff':'#58ffd1';ctx.fillStyle=t.hit>0?'#fff':'#58ffd1';ctx.beginPath();ctx.arc(0,0,17,0,7);ctx.fill();ctx.shadowBlur=0;ctx.fillStyle='#142421';ctx.fillRect(-4,-5,31,10);ctx.strokeStyle=t.hp===1?'#ff9d3d':'#8affdf';ctx.lineWidth=3;ctx.beginPath();ctx.arc(0,0,22,0,7);ctx.stroke();ctx.restore();ctx.fillStyle='#20282e';ctx.fillRect(t.x-22,t.y-32,44,4);ctx.fillStyle=t.hp===1?'#ff9d3d':'#58ffd1';ctx.fillRect(t.x-22,t.y-32,44*t.hp/t.maxHp,4)});
  ...
  bullets.forEach(b=>{ctx.strokeStyle=b.special?'#43cfff':b.reflected?'#e8f7ff':b.ally?'#75e7ff':'#dfff86';ctx.lineWidth=b.special?8:b.reflected?6:4;ctx.shadowBlur=b.special||b.reflected?18:0;ctx.shadowColor=b.reflected?'#9ee7ff':'#43cfff';ctx.beginPath();ctx.moveTo(b.x,b.y);ctx.lineTo(b.x-b.vx*.018,b.y-b.vy*.018);ctx.stroke();ctx.shadowBlur=0});
  ...
  ctx.fillStyle=turrets.length>10?'#ff405d':'#84918d';ctx.font='700 10px JetBrains Mono';ctx.fillText(`${tr('AKTYWNE WIEŻYCZKI','ACTIVE TURRETS')}: ${turrets.length} / 11`,W-288,H-105);
