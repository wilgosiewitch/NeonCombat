...
function reset(){
  ...
  shopOpen=false;pauseOpen=false;turretHpMultiplier=1;playerDamageMultiplier=1;healthRegen=false;ultraTurretPurchased=false;fireTrailPurchased=false;rapidFirePurchased=false;dualShotPurchased=false;playerHpPurchased=false;shopScreen.classList.add('hidden');pauseScreen.classList.add('hidden');
...
function spawnBoss(){
  ...
  enemies.push({x:p.x,y:p.y,r:48,hp:250,maxHp:250,speed:0,shoot:99,laserAngle:0,laserSound:0,closeBeam:0,closeCooldown:0,closeAngle:0,angle:0,tank:false,elite:false,boss:true,hit:0});
...
function useRoleAbility(){
  ...
  if(chosenRole==='madman'){
    ...
    enemies.forEach(e=>{
      ...
      if(e.hp<=0){defeated++;credits+=energyReward(e.boss?500:e.elite?150:e.tank?60:30);score+=e.boss?3000:e.elite?750:e.tank?300:100;kills++;if(e.elite)eliteRespawn=20;if(e.boss){playRedBossDeadSound();bossFightActive=false;bossPurgeTimer=0;spawnTimer=.25;eliteRespawn=1}}
    });
...
function rewardChainedDefeat(e){
  ...
  if(e.boss){playRedBossDeadSound();bossFightActive=false;bossPurgeTimer=0;spawnTimer=.25;eliteRespawn=1;notice=tr('TYTAN POKONANY — FALE WZNOWIONE','TITAN DEFEATED — WAVES RESUMED');noticeTime=3}
...
function endGame(){if(!running)return;running=false;stopInGameMusic();gameOverSound.currentTime=0;gameOverSound.play().catch(()=>{});finalStats.textContent=tr(`Wynik: ${score.toLocaleString('pl-PL')}  •  Pokonani: ${kills}  •  Fala: ${wave}`,`Score: ${score.toLocaleString('en-US')}  •  Defeated: ${kills}  •  Wave: ${wave}`);gameOverScreen.classList.remove('hidden')}
...
function begin(){stopLobbyMusic();stopInGameMusic();gameOverSound.pause();gameOverSound.currentTime=0;reset();running=true;startScreen.classList.add('hidden');gameOverScreen.classList.add('hidden');inGameMusicActive=true;playInGameMusic()}
...
document.addEventListener('click',e=>{
  ...
  if(!button)return;
  if(button.closest('.offer'))playShopBuyingSound();
  else playMenuClickSound();
});
addEventListener('keydown',e=>{if(e.key==='Escape'&&!e.repeat){togglePause();e.preventDefault();return}if(pauseOpen)return;if(e.key.toLowerCase()==='b'&&!e.repeat){toggleShop();return}if(shopOpen)return;keys[e.key.toLowerCase()]=true;if(e.key===' '&&!e.repeat)spaceDashQueued=true;if(e.key.toLowerCase()==='z'&&!e.repeat)turretQueued=true;if(e.key.toLowerCase()==='e'&&!e.repeat)healQueued=true;if(['ArrowUp','ArrowDown','ArrowLeft','ArrowRight',' '].includes(e.key))e.preventDefault()});
...
setLanguage('pl');fitGameToScreen();reset();running=false;playLobbyMusic();requestAnimationFrame(loop);
